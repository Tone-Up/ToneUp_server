...
public class DiaryController {
    ...
    @GetMapping("/my-diary")
    public ResponseEntity<?> getDiaryPagination(@RequestParam(required = false) Long cursor,
            @RequestParam(required = false) Integer limit, @AuthenticationPrincipal CustomOAuth2User customOAuth2User) {
        Long userId = customOAuth2User.getId();
