...
### 임베딩 파일 전체 올려서 힙 메모리 부족 oom 발생 위험=> 스트리밍 방식으로 해결 + 레디스 파이프라이닝

임베딩 파일 전체를 한번에 읽어서 레디스 저장 중 힙 메모리 부족으로 인한 oom 발생 위험

스트리밍 방식으로 나눠서 메모리에 올려 문제 해결 + 레디스 파이프라이닝 적용을 통해 성능 향상
