...
- **행동**: Streaming Parser 도입: 파일 전체를 메모리에 올리지 않고 데이터 일부를 읽어 처리하는 Jackson JsonParser 기반의 스트리밍 방식을 채택하여 메모리 점유율을 최소화했습니다.
...
---
### CloudFront를 활용한 정적 리소스(이미지) 응답 속도 및 비용 최적화 (60ms → 35ms, 41% 향상)

**상황** : 서비스 내 프로필 및 게시글 이미지를 S3에서 직접 호출하여 사용자에게 제공하고 있었습니다.

**문제** : 전 세계/전 지역 사용자에게 동일한 속도를 보장하기 어렵고, S3 직접 액세스로 인한 데이터 전송 비용(Data Transfer Out) 부담과 네트워크 지연(Latency)이 발생했습니다.

**행동** : CloudFront(CDN)를 도입하여 이미지를 캐싱했습니다. S3 버킷을 비공개(Private)로 전환하고 OAC(Origin Access Control)를 설정하여 보안을 강화했습니다.
캐시 정책(Cache Policy) 최적화를 통해 S3 호출 빈도를 최소화했습니다.

**결과** : 이미지 조회 성능을 약 41% 개선하였으며, S3 직접 호출 대비 저렴한 CloudFront 전송 비용을 통해 운영 비용을 절감했습니다.
...
~~- **행동**: 사용자가 마지막으로 읽은 지점을 추적하는 lastReadMessageId 컬럼을 추가하고, 이를 활용해 인덱스 기반의 범위 카운트(Range Scan) 서브쿼리 방식으로 로직을 변경했습니다.~~
